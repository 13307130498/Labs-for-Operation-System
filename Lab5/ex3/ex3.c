...
int FIFOFrameNumber;
int successfulAccess, TLBMiss, PageFault;
...
int main()
{
    ...
    FIFOFrameNumber = 0;
    successfulAccess = TLBMiss = PageFault = 0;
    ...
    printf("%.2f\n", 100.0 * TLBMiss / successfulAccess);
    //    3. Percentage of Page-Fault (2 place of precision)   
    printf("%.2f\n", 100.0 * PageFault / successfulAccess);    
...
int handlePageFault( OS* os, int pageNum, int* victimPageNum )
{
    ...
    frameNumber = FIFOFrameNumber;
    FIFOFrameNumber = (FIFOFrameNumber + 1) % 8;
    ...
    *victimPageNum = replacedPageNum;
    ...
    if ( replacedPageNum != -1 ){
        ...
        if(os->physicalMem->IsDirty[frameNumber] == 1){
            readOneFrame(os->physicalMem, &tempPage, frameNumber);
            writeSwapPage(os->secStorage, &tempPage, replacedPageNum);
        }
